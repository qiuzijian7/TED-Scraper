...
def download(urls, id_, csv_list):
    ...
    for count, url in enumerate(urls):
        ...
        def get__json_obj(url):
            ...
            while html=='':
                try:
                    ...
                    logging.info("Let me sleep for 5 seconds")
                    logging.info("ZZzzzz...")
                    time.sleep(5)
        ...
        if not json_obj:    
            logging.info('url failed:', url);
            continue;
        else:  
            logging.info('url success:', url);        
...
if __name__ == '__main__':
    ...
    logging.info(round(time.time()  -  time__))
